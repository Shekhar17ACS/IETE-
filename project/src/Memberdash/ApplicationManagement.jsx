...
export default function ApplicationManagement() {
  ...
  const handleToggleStep = async (candidateId, step, currentStatus) => {
    ...
    try {
      ...
      const response = await updateApplicationVerification(
        verificationData,
        token
      );
      if (response.success !== false) {
        toast.success(
          `Updated ${step.replace(/([A-Z])/g, " $1").trim()} status`
        );
        const updatedApplications = await getApplications(token);
        setCandidates(updatedApplications);
        setSelectedCandidate(
          updatedApplications.find((c) => c.id === candidateId) || null
        );
      } else {
        throw new Error(
  ...
  const filteredCandidates = candidates.filter((candidate) => {
    ...
    const matchesStatus =
      ...
      (statusFilter === "pending_payment" &&
        candidate.applicationSteps.payment.status === "Pending") ||
      (statusFilter === "eligibility_pending" &&
        candidate.eligibility.status === "Pending") ||
      (statusFilter === "complete" &&
        Object.values(candidate.applicationSteps).every(
          (step) => step.completed
        ));
    ...
    const matchesDate =
      !filterDate ||
      new Date(
        candidate.applicationSteps.personalDetails.timestamp
      ).toDateString() === new Date(filterDate).toDateString();
    ...
    <div className="bg-white rounded-xl p-6 shadow-sm">
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-xl font-semibold text-gray-800">
          {candidate.name} - Application #{candidate.id}
        </h2>
        <button
      ...
      <div className="space-y-6">
        <div>
          ...
          {candidate.city && (
            <p className="text-gray-600">City: {candidate.city}</p>
          )}
          {candidate.state && (
            <p className="text-gray-600">State: {candidate.state}</p>
          )}
          {candidate.country && (
            <p className="text-gray-600">Country: {candidate.country}</p>
          )}
        </div>
            ...
            candidate.academic.map((qual, index) => (
              <div key={index} className="text-gray-600 mb-2">
                ...
                {qual.percentage_cgpa && (
                  <p>Percentage/CGPA: {qual.percentage_cgpa}</p>
                )}
                {qual.document && (
                  <a
                    href={qual.document}
        ...
        <div>
          ...
          {candidate.experience.length > 0 ? (
            candidate.experience.map((exp, index) => (
              <div key={index} className="text-gray-600 mb-2">
                <p>
                  {exp.job_title} at {exp.organization_name}
                </p>
                <p>
                  {exp.work_type} ({exp.employee_type})
                </p>
                <p>
        ...
        <div>
          ...
          <p className="text-gray-600">
            Start Date:{" "}
            {format(new Date(candidate.membership.startDate), "MMM d, yyyy")}
          </p>
        </div>
        ...
        <div>
          ...
          <div className="space-y-2">
            {Object.entries(candidate.applicationSteps).map(
              ([step, data], index) => (
                <div key={step} className="flex items-center gap-3">
                  ...
                  <button
                    onClick={() =>
                      handleToggleStep(candidate.id, step, data.completed)
                    }
                  ...
                  >
                    ...
                    <span>
                      {data.completed ? "Approved" : "Approve Manually"}
                    </span>
                  </button>
                </div>
              )
        ...
        <div>
          ...
          <p className="text-gray-600">
            Status: {candidate.applicationSteps.payment.status}
          </p>
