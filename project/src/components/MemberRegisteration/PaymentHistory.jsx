...
import { fetchPaymentHistory, setCurrentPage } from "../Redux/ReduxSlice/paymentSlice"
import {
  Download,
  Calendar,
  CreditCard,
...
const PaymentHistory = ({ token }) => {
  ...
  const { payments, count, next, previous, currentPage, loading, error } = useSelector((state) => state.payment)
  ...
  const downloadReceipt = async (payment) => {
    try {
      ...
      doc.setFont("helvetica", "bold")
      doc.setFontSize(14)
      doc.text("Total Amount Paid", pageWidth / 2, yPos + 12, { align: "center" })

      doc.setFontSize(20)
      doc.text(formatAmount(payment.amount, payment.currency), pageWidth / 2, yPos + 22, { align: "center" })
      ...
      doc.text("This is an automatically generated receipt.", pageWidth / 2, pageHeight - 18, { align: "center" })
      ...
      // Save the PDF
      const filename = `Receipt-${payment.order_id}-${getIdSubstring(payment.id, 0, 6)}.pdf`
      doc.save(filename)
                      ...
                      <tr key={payment.id} className="hover:bg-blue-50 transition-colors duration-200">
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            <div className="text-sm font-bold text-gray-900">#{payment.id}</div>
                            <div className="text-sm text-gray-600 font-medium">Order ID: {payment.order_id}</div>
                            <div className="text-xs text-gray-500 mt-1">{formatDate(payment.created_at)}</div>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex items-center">
                            ...
                            <span className="text-sm font-semibold text-gray-900">{payment.membership_type}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs text-gray-500 font-medium">{payment.currency}</span>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div
                ...
                return (
                  ...
                  >
                    <div className="flex justify-between items-start mb-4">
                      <div>
                        <div className="text-sm font-bold text-gray-900">#{payment.id}</div>
                        <div className="text-xs text-gray-600">Order: {payment.order_id}</div>
                      </div>
                      <div
